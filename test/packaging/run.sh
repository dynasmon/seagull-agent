...
main() {
  ...
  for required in seagull-agent install.sh upgrade.sh rollback.sh uninstall.sh VERSION MANIFEST.sha256 README.md LICENSE \
    lib/common.sh lib/layout.sh lib/envfile.sh lib/artifact.sh lib/service.sh lib/state.sh lib/migrate.sh \
    share/systemd/seagull-agent.service share/env/seagull-agent.env.example share/protocol-v1.json; do
  ...
  check "install succeeds" env SEAGULL_INSTALL_ROOT="${root}" bash "${pkg1}/install.sh" \
    ...
    --enroll-url https://siem.example.com:8445 --enroll-token abt.agent-test-1.token --no-start
  ...
  check "explicit capability override is honoured" env SEAGULL_INSTALL_ROOT="${root}" bash "${pkg1}/install.sh" --profile managed --capabilities none --no-start
  check "override removed all capabilities" file_lacks "${dropin}" 'CAP_'
  check "restore managed capabilities" env SEAGULL_INSTALL_ROOT="${root}" bash "${pkg1}/install.sh" --profile managed --no-start
